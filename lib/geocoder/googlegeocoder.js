...
var _get = (obj, prop) => get(obj, prop, null)
...
GoogleGeocoder.prototype._formatResult = function (result) {
  ...
  extractedObj.country = _get(components, 'country.long_name')
  extractedObj.countryCode = _get(components, 'country.short_name')
  extractedObj.city = _get(components, 'locality.long_name') || _get(components, 'postal_town.long_name')
  extractedObj.zipcode = _get(components, 'postal_code.long_name')
  extractedObj.streetName = _get(components, 'route.long_name')
  extractedObj.streetNumber = _get(components, 'street_number.long_name')
  extractedObj.extra.premise = _get(components, 'premise.long_name')
  extractedObj.extra.subpremise = _get(components, 'subpremise.long_name')
  extractedObj.extra.establishment = _get(components, 'establishment.long_name')
  // extractedObj.neighborhood = _get(components, 'neighborhood.long_name')

  var sublocality = _get(components, 'sublocality_level_1.long_name') || _get(components, 'sublocality.long_name') || _get(components, 'neighborhood.long_name') || _get(components, 'locality.long_name')
  var localities = [_get(components, 'locality.short_name'), _get(components, 'administrative_area_level_2.short_name'), _get(components, 'administrative_area_level_1.short_name')]
  var shortestLocality  = localities.reverse().filter(Boolean).reduce((a,b) => a.length < b.length ? a : b)
  extractedObj.neighborhood = [sublocality, shortestLocality].filter(Boolean).join(', ')
  ...
  //       case 'sublocality':
  //       case 'neighborhood':
  //         if(!extractedObj.neighborhood) {
  //           extractedObj.neighborhood = result.address_components[i].long_name;
  //         }
  //         break;
  //     }
  //   }
